...
int timer_set_square(unsigned long timer, unsigned long freq) {
	...
	if((st>>1) & (BIT(0)|BIT(1)|BIT(2)) == 3)
		setDiv = timerInfo | TIMER_LSB_MSB | ((BIT(3)|BIT(2)|BIT(1)|BIT(0)) & st);
	else
		setDiv = timerInfo | TIMER_LSB_MSB | TIMER_SQR_WAVE | (BIT(0) & st);
